...
export function QuickNoteBox({ practiceContext }: QuickNoteBoxProps) {
  ...
  if (!isExpanded) {
    return (
      <button
        ...
        className="fixed right-4 bottom-1/4 z-30 bg-white border-2 border-gray-200 rounded-xl px-3 py-2 shadow-lg hover:shadow-xl hover:border-cyan-300 transition-all group"
        title="Add a note about yourself"
      >
        <div className="flex items-center gap-2">
          <span className="text-lg">📝</span>
          <span className="text-sm text-gray-600 group-hover:text-cyan-600 hidden sm:inline">
            About me
          </span>
        </div>
      </button>
  ...
  if (showSuccess) {
    return (
      <div className="fixed right-4 bottom-1/4 z-30 bg-green-50 border-2 border-green-300 rounded-xl p-4 shadow-lg w-64">
  ...
  return (
    <div className="fixed right-4 bottom-1/4 z-30 bg-white border-2 border-cyan-200 rounded-xl shadow-xl w-72">
      {/* Header */}
      ...
      <div className="p-3">
        <textarea
          ...
          rows={3}
          className="w-full resize-none border border-gray-200 rounded-lg p-2 text-sm focus:ring-2 focus:ring-cyan-500 focus:border-transparent"
          disabled={isSaving}
        />
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-black text-white text-sm rounded-lg hover:bg-gray-800 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
          >
