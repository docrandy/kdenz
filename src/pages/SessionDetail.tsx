    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="text-center">
          <h1 className="text-xl font-semibold text-text mb-2">Session Not Found</h1>
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 bg-background border-b border-background-elevated z-10">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold text-text">Session Details</h1>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 py-6">
        {/* Date/Time */}
        <div className="mb-6">
          <p className="text-sm text-text-muted">Session recorded</p>
          <p className="text-lg font-medium text-text">{formatDate(session.timestamp)}</p>
        </div>

        {/* Primary Metric */}
        <div className="bg-background-surface rounded-xl p-6 mb-6">
          <p className="text-sm text-text-muted mb-1">Filler Rate</p>
          <p className={`text-4xl font-bold ${getFillerRateColor(session.fillerRate)}`}>
            {session.fillerRate.toFixed(1)}%
          </p>
          <p className="text-sm text-text-muted mt-1">fillers per 100 words</p>
        </div>

        {/* Secondary Metrics */}
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-background border border-background-elevated rounded-xl p-4 text-center">
            <p className="text-2xl font-semibold text-text">{session.wpm}</p>
            <p className="text-xs text-text-muted">WPM</p>
          </div>
          <div className="bg-background border border-background-elevated rounded-xl p-4 text-center">
            <p className="text-2xl font-semibold text-text">{session.wordCount}</p>
            <p className="text-xs text-text-muted">Words</p>
          </div>
          <div className="bg-background border border-background-elevated rounded-xl p-4 text-center">
            <p className="text-2xl font-semibold text-text">{session.fillerCount}</p>
            <p className="text-xs text-text-muted">Fillers</p>
          </div>
        </div>
        ...
        <div className="bg-background border border-background-elevated rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm font-medium text-text">{formatDuration(session.durationSeconds)}</span>
          </div>
        </div>

        {/* Note about playback */}
        <div className="text-center text-sm text-text-subtle mt-8">
