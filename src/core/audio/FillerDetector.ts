/**
 * EPIC 3.1: Filler Detection (Post-Turn Only)
 *
 * Purpose: Detect filler words without live callouts
 * - Local keyword spotting: um, uh, like, you know
 * - Acoustic pattern matching (MVP approach)
 * - ONNX model integration ready
 *
 * Rules:
 * - NEVER display during speech
 * - Show only post-turn or post-session
 * - Label as "estimate"
 *
 * Done when:
 * - Precision ≥70% on test set
 * - User can toggle filler metric off
 */
 ...
 * Future: Can be enhanced with ONNX KWS model
 */
export class FillerDetector {
  private config: FillerConfig;
  private audioContext: AudioContext;
  private analyserNode: AnalyserNode;
  // Note: Using 'any' to work around strict TypeScript lib.dom.d.ts typing
  // AnalyserNode methods expect specific ArrayBuffer types
  ...
  /**
   * Get current detections (for real-time count)
   */
  getCurrentDetections(): FillerDetection[] {
    return [...this.detections];
  }

  /**
   * Get current count (for real-time display)
   */
  ...
  private processAudio = (): void => {
    ...
    // Cast required due to TypeScript lib.dom.d.ts strict ArrayBuffer typing
